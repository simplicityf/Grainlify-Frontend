...
describe('BlogPage – loading state', () => {
  it('shows a loading skeleton while fetching', () => {
    ...
    expect(loadingRegion).toHaveAttribute('aria-label', 'Loading blog posts')

    // Verify themed skeleton loaders are rendered (with shimmer animation class)
...
describe('BlogPage – error state', () => {
  ...
  it('retries the fetch when the Retry button is clicked', async () => {
    ...
    mockGetBlogPosts.mockRejectedValueOnce(new Error('fail')).mockResolvedValueOnce([
      {
        id: 1,
        slug: 'first-post',
        title: 'First Post',
...
describe('BlogPage – ok state', () => {
  ...
  it('renders the featured post and recent posts grid', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Featured Post Title')).toBeInTheDocument())
...
describe('FeaturedPost – missing post fallback', () => {
  it('shows a fallback when the API returns only one post (no recent posts)', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Only Post')).toBeInTheDocument())
    // No recent posts – empty state inside the grid.
    expect(screen.getByText(/no recent articles available/i)).toBeInTheDocument()
  })
})
